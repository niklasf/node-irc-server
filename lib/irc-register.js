...
exports.extend = function (server) {
	...
	server.register = new EventManager({
		...
		function (client) {
			...
			if (client.registered) {
				...
				// do not immediately delete server.register.clients[scand]
			}
		...
		function (client) {
			...
			// Remove the client and all aliases
			for (var key in server.register.clients) {
				if (server.register.clients.hasOwnProperty(key)) {
					if (server.register.clients[key] === client) {
						delete server.register.clients[key];
					}
				}
			}
			
			assert.strictEqual(server.register.clients[
					client.name.toScandinavianLowerCase()],
					undefined, 'Removed from client list');
			
			server.log.debug(client.remoteAddres, client.name,
					'Client unregistered');
		},
	...
	server.commands.override('QUIT', function (client, message) {
		// Unregister clients when they quit
		this.next(client, message);
		server.register.negotiate('unregister', client);
	});
	...
	function (client, username, hostname, servername, realname) {
		...
		// Apply hostname only if not yet looked up
