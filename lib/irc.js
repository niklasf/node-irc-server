...
exports.createServer = function (options) {
	...
	connectionListener = function (client) {
		...
		// Perform reverse hostname lookup
		if (net.isIP(client.remoteAddress)) {
			dns.reverse(client.remoteAddress, function (err, domains) {
				if (err !== null || domains.length === 0) {
					log.warn(client.remoteAddress, client.name,
						'Reverse hostname lookup failed:', err, domains);
				} else {
					client.domains = domains.length;
					client.hostname = domains[0];
					log.info(client.remoteAddress, client.name,
						'Hostname lookup performed:', domains);
				}
			});
		} else {
			log.info(client.remoteAddress, client.name,
					'Performing no hostname lookup');
		}
