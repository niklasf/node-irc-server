...
exports.deliver = function (from, command, args) {
...
exports.createServer = function (options) {
			...
			try {
				...
				if (error === 'ERR_NEEDMOREPARAMS') {
					...
					if (server.commands.implemented(command)) {
						...
						// Command not found
						error = 'ERR_UNKOWNCOMMAND';
						client.deliver(server.name, 421,
								[client.name, command, 'Unkown command']);
					}
				} else {
					throw error;
				}
	...
	server.creation = tools.unixtime();
	server.usermodes = '';
	server.channelmodes = '';
	server.motd = ['Welcome.'];
	...
	server.registerClient = function (client, oldName) {
		var name = client.name.toScandinavianLowerCase(),
			current = server.clients[name];
		
		// Revert nickname if in use
		if (current && current.name.toScandinavianLowerCase() === name) {
			log.info(client.remoteAddress, oldName,
					'Nickname was already in use:', client.name);
			client.deliver(server.name, 433,
					[oldName, client.name, 'Nickname is already in use']);
			client.name = oldName;
			return false;
		}
		
		// Register client
		log.info(client.remoteAddress, client.name,
				'Registered client with old name:', oldName);
		client.registered = true;
		server.clients[name] = client;
		return true;
	};
	
	server.findClient = function (name) {
		return server.clients[name.toScandinavianLowerCase()];
	};
	...
	server.joinChannel = function (client, channel) {
		// TODO: Implement
	};
	
	server.partChannel = function (client, channel) {
		// TODO: Implement
	};
	...
	require('./irc-register.js').extend(server);
