...
exports.createServer = function(options) {
		...
		client.on('command', function(command, args) {
			if(client.registered) {
				try {
					...
					log.warn(client.remoteAddress,
							client.name, 'Unkown command:', command, args);
				}
			} else {
				try {
					...
					if(server.commands.implemented(command)) {
						...
						log.warn(client.removeAddress, client.name,
							'Was not registered.', command, args);
					} else {
						client.deliver(server.name, client.name, 421,
								[command, 'Unknown command']);
						log.warn(client.remoteAddress,
	...
	// TODO: Read about channel and client modes
